...
public partial class ScheduleMonthControl : UserControl
{
    ...
    private static readonly int SlotMinutes = 30;
    ...
    private void Rebuild()
    {
        ...
        // For each employee: title row + slot rows
        foreach (var emp in mv.Employees.Where(e => e.IsActive))
        {
            // Employee title row (spans across)
            ...
            for (int s = 0; s < slots.Count; s++)
            {
                ...
                for (int d = 0; d < days.Count; d++)
                {
                    ...
                    // Find appointments/absences for this employee+date that intersect this slot
                    ...
                    {
                        // Render first item as a “chip”-like block
            ...
            // small spacer row between employees
    ...
    private List<CellItem> FindItemsForSlot(MonthView mv, string employeeId, DateOnly date, TimeOnly slotStart, int slotMinutes)
    {
        ...
        // MonthView.Cells already contains day cells (for all employees merged). We filter here:
    ...
    private Brush TryFindBrush(string key)
    {
        ...
        return key switch
        {
            "HeaderBlue" => new SolidColorBrush(Color.FromRgb(33, 115, 70)), // fallback
    ...
    private static Brush StatusToBrush(AppointmentStatus s)
        => s switch
        {
            AppointmentStatus.Fixed => new SolidColorBrush(Color.FromRgb(46, 125, 50)),      // green
            AppointmentStatus.Tentative => new SolidColorBrush(Color.FromRgb(245, 124, 0)),  // orange
            AppointmentStatus.Cancelled => new SolidColorBrush(Color.FromRgb(211, 47, 47)),  // red
            _ => new SolidColorBrush(Color.FromRgb(25, 118, 210)),                            // blue
        };
